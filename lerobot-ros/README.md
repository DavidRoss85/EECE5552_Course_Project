...
## Quickstart with UR12e (Gazebo + MoveIt Servo)

This project includes a `ur12e_ros` robot config and `ros_twist` teleoperator for the UR12e in Gazebo. The `ros_twist` teleop forwards `geometry_msgs/Twist` from a topic (e.g. `/game_controller` from joy + teleop_twist_joy) as 6-DOF actions.

**Prerequisites:** EECE5552_Course_Project sim (Gazebo, MoveIt, Servo), joy + teleop_twist_joy publishing to `/game_controller`.
...
# Terminal 3: LeRobot (conda env)
conda activate lerobot-ros
source /opt/ros/jazzy/setup.bash
lerobot-teleoperate --robot.type=ur12e_ros --teleop.type=ros_twist
```

See [docs/UR12E_LEROBOT.md](../docs/UR12E_LEROBOT.md) in the course project for full setup and recording.
