...
class GamepadController6DOF(InputController):
    ...
    def get_6dof_deltas(self):
        ...
        # Trigger deadzone: analog triggers often don't return exactly 0 at rest,
        # causing constant Z drift. Use a slightly larger deadzone for the net value.
        trigger_net = right_trigger - left_trigger
        trigger_net = 0 if abs(trigger_net) < self.deadzone else trigger_net

        # Calculate deltas
        # Linear movement
        linear_x = -left_x * self.x_step_size  # Forward/backward
        linear_y = left_y * self.y_step_size  # Left/right
        linear_z = trigger_net * self.z_step_size  # Up/down
