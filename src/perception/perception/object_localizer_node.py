...
# Gripper pointing straight down in world frame: 90-deg rotation around Y axis
...
class ObjectLocalizerNode(Node):
    def __init__(self):
        ...
        if not self._use_depth:
            self.get_logger().warn(
                'ObjectLocalizerNode: use_depth=false — no /goal_pose will be published')
            return
        ...
        self.create_subscription(Point, '/vla/target_coords', self._selected_cb, 10)

        self.get_logger().info('Object localizer ready (depth enabled)')
    ...
    def _selected_cb(self, msg: Point):
        ...
        try:
            ...
            self.get_logger().info(f'Goal: x={p.x:.3f} y={p.y:.3f} z={p.z:.3f}')
